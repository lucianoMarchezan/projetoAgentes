...
class ResponderChamado(TimedBehaviour):
    ...
    def on_time(self):
        ...
        if(self.agent.casaChamado != None):
            ...
            self.agent.casaChamado = Casa(
                casaNova['linha'], casaNova['coluna'])
        else:
    ...
    def moverAteLocal(self):
        ...
        if (proximaCasa != None and self.beliefVerificarCasa(proximaCasa)):
            requests.post("http://localhost:9000/ApagarFogo",
                          json=proximaCasa.returnJsonObject())
        ...
        if proximaCasa != None and self.verificarFimChamado(proximaCasa):
            ...
            display_message(self.agent.aid.localname,
                            'Movendo para!' + proximaCasa.nameId)
